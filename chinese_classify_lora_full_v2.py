'''
    ...
    弃用了之前标签有误的THUCNews-mini数据集中的train.txt和test.txt数据集，采用了最新的THUCNews-mini-full数据集进行重新训练和验证。
...
def verify_model_loading(model, test_text, id_to_label):
    """验证模型是否正确加载"""
    pred, _ = predict_category(model, tokenizer, test_text, device, id_to_label)
    print(f"预测结果: {pred}")
...
if __name__ == "__main__":
    ...
    train_loader, val_loader, tokenizer, label_to_id, id_to_label = prepare_datasets(os.path.join(model_dir, model_name), train_data_path)
    # 打印一个批次数据
    print("\n训练集批次示例:")
    ...
    # 执行测试
    test_model_lora(test_model, test_loader, device, test_df, high_entropy_samples_dir=HIGH_ENTROPY_SAMPLES_DIR, id_to_label=id_to_label)
    ...
    for sample in test_samples:
        pred_label, pred_probs = predict_example(test_model, tokenizer, sample, device, id_to_label)
        print(f"文本: {sample}")
