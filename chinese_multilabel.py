...
if __name__ == "__main__":
    ...
    for epoch in range(EPOCHS):
        ...
        if val_acc > best_accuracy:
            torch.save(model.state_dict(), 'best_text_model.bin')
    ...
    #     category, probs = predict_category(model, tokenizer, review, device)
    #     print(f"Review: {review}")
    #     print(
    #         f"Sentiment: {category} | Probabilities: Negative={probs[0]:.2f}, Neutral={probs[1]:.2f}, Positive={probs[2]:.2f}")
    #     print("-" * 50)
    ...
    print(f"Totally train time: {elapsed_time} secs!")
    ...
    #     tokenizer=tokenizer,
    #     device=device,
    #     initial_samples=200,  # 初始200个样本
    #     iterations=5,  # 5轮主动学习
    #     samples_per_iter=50  # 每轮新增50个样本
    # )
    ...
    test_df = load_THUCNews_test('./data/THUCNews-mini/test.txt')
    ...
    # 加载训练好的权重
    model.load_state_dict(torch.load('best_text_model.bin'))
    model = model.to(device)
    print("\n已加载最佳模型权重")
    ...
    # === 输出最大熵测试样本
    test_model_v2(model, test_loader, device, test_df, output_dir='./test_results', )
    ...
    for sample in test_samples:
        pred_label, pred_probs = predict_example(model, tokenizer, sample, device)
        print(f"文本: {sample}")
