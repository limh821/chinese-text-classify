'''
    ...
    3、基础模型和分类头和LoRa适配器一起合并保存（（具体保存地址在：{output_dir}/deployment_model/））以保证测试时重新正确加载，单独分离保存基础模型（含分类头）无法保证测试时重新正确加载（具体表现是测试加载时候报分类头参数初始化异常，
       而且测试集准确率远低于验证集准确率，原因见下面的“重要补充说明”）
    4、LoRa适配器另外单独保存以用于后续训练（具体保存地址在：{output_dir}/training_adapter/）
...
def train_and_evaluate_lora(model, train_loader, val_loader, output_dir, num_epochs=3, learning_rate=1e-4):
    ...
    print("---")
    print(f"完成模型训练！")
    print(f"最佳检查点的epoch数为 best_checkpoint_epoch : {BEST_CHECKPOINT_EPOCH}，最佳检查点保存路径为 {output_dir}/training_adapter/ ")
...
def save_merged_model(model, output_dir):
    ...
    # 保存完整模型
    merged_model.save_pretrained(os.path.join(output_dir, "deployment_model"))
    tokenizer.save_pretrained(os.path.join(output_dir, "deployment_model"))
    print(f"部署模型已保存到 {output_dir}/deployment_model")
...
# 恢复训练示例
def continue_training(output_dir, best_checkpoint_epoch):
    # todo ... （待完善）
    # 加载上次保存的最佳适配器
    adapter_path = f"{output_dir}/best_epoch_{best_checkpoint_epoch}/training_adapter"

    # 加载基础模型（原始预训练模型）
    base_model_path = model_dir + model_name

    # 恢复模型
    model = resume_training(adapter_path, base_model_path, num_labels)
    ...
    # 继续训练
    train_and_evaluate_lora(model, train_loader, val_loader)
...
# 包含最大熵样本统计和输出的测试函数
def test_model_lora(model, test_loader, device, test_df, high_entropy_samples_dir):
    """
    增强版测试评估函数
    功能：
    1. 计算标准测试指标
...
if __name__ == "__main__":
    ...
    # === 输出最大熵测试样本
    test_model_lora(test_model, test_loader, device, test_df, high_entropy_samples_dir=HIGH_ENTROPY_SAMPLES_DIR)
